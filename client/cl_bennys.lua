...
local function saveVehicle()
    ...
    TriggerServerEvent('script-customs:server:updateVehicle', myCar, "customs")
end
...
function AllowJob(restrictionData, job)
...
function AllowGang(restrictionData, gang)
...
function AllowVehicleClass(restrictionData, vehicle)
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("script-customs:server:attemptPurchase", type, upgradeLevel)
...
function EnterLocation(override)
    ...
    local image = (locationData and locationData.settings.image) or "bennys"
    ...
    SetTimeout(100, function()
        ...
        DisplayMenuContainer(true, image)
...
function GetLocations()
    QBCore.Functions.TriggerCallback("script-customs:server:GetLocations", function(locations)
...
function SetupInteraction()
    ...
    if not radialMenuItemId then
        radialMenuItemId = exports['qb-radialmenu']:AddOption({
            ...
            icon = 'wrench',
            type = 'client',
            event = 'script-customs:client:EnterCustoms',
            shouldClose = true
        })
    end
        ...
        for i, spot in ipairs(data.zones) do
            ...
            newSpot:onPlayerInOut(function(isPointInside, point)
                if isPointInside then
                    -- print(location)
                    -- print(_name)
                    -- print(vector3(spot.coords.x, spot.coords.y, spot.coords.z))
                    -- print(spot.heading)
                    -- print(data.drawtextui.text)
                    CustomsData = {
                        ['location'] = location,
                        ['spot'] = _name,
...
RegisterNetEvent('script-customs:client:UpdateLocation', function(location, type, key, value)
    Config.Locations[location][type][key] = value
end)

RegisterNetEvent("script-customs:client:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    QBCore.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("script-customs:client:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    QBCore.Functions.Notify("Not enough money", "error")
end)

RegisterNetEvent('script-customs:client:EnterCustoms', function(override)
    if not override.coords or not override.heading then
        override = nil
    end
    if not IsPedInAnyVehicle(PlayerPedId(), false) or isPlyInBennys or (not next(CustomsData) and not override) then
        return
    end
    if not override and next(CustomsData) and not CheckRestrictions(CustomsData.location) then
        return
    end
...
RegisterNetEvent('script-customs:client:EnterCustomsAdmin', function()
    local categories = {
        repair = true,
        mods = true,
        armor = true,
